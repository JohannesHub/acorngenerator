...
func main() {
	if len(os.Args) != 3 {
		failErr(fmt.Errorf("expected exactly two arguments: <source type> <source interface>"))
	}

	goPackage := os.Getenv("GOPACKAGE")

	sourceStructName, _, structObject := getType(os.Args[1])
	...
	sourceInterfaceName, sourceInterfacePackage, interfaceObject := getType(os.Args[2])

	interfaceType, ok := interfaceObject.Type().Underlying().(*types.Interface)
	if !ok {
		failErr(fmt.Errorf("type %v is not an interface", interfaceObject))
	}

	err := generate(sourceStructName, structType, sourceInterfaceName, interfaceType, sourceInterfacePackage, goPackage)
	if err != nil {
		failErr(err)
	}
}

func getType(param string) (string, string, types.Object) {
	...
	return sourceTypeName, sourceTypePackage, obj
}

func generate(sourceTypeName string, structType *types.Struct, sourceInterfaceName string, sourceInterfaceType *types.Interface, sourceInterfacePackage, goPackage string) error {
	f := NewFile(goPackage)
	...
	f.Func().Params(Id("s").Id("*" + sourceTypeName)).Id("AcornName").Params().String().Block(
		Return(Qual(sourceInterfacePackage, sourceInterfaceName+"AcornName"))).Line()
	...
	setupStatements := generateSetupStatements(injectedFields, sourceInterfaceName, sourceInterfaceType)
...
func generateSetupStatements(injectedFields []*types.Var, sourceInterfaceName string, sourceInterfaceType *types.Interface) []Code {
	setupStatements := generateInjectedFieldsSetup(injectedFields)

	if !hasSetupMethod(sourceInterfaceType) {
	...
	loggingStatement := Id("ctx").Op(":=").
		...
		Id("s").Dot("Logging").Dot("Logger").Call().Dot("Ctx").Call(Id("ctx")).
			...
			Call(Id("\"failed to set up " + sourceInterfaceName + ". BAILING OUT\"")).
			Line().Return(Err())).Line().
		Id("s").Dot("Logging").Dot("Logger").Call().Dot("Ctx").Call(Id("ctx")).
		Dot("Info").Call().Dot("Print").Call(Id("\"successfully set up " + sourceInterfaceName + "\""))
	setupStatements = append(setupStatements, loggingStatement)
	return append(setupStatements, Return(Nil()))
}

func hasSetupMethod(sourceInterfaceType *types.Interface) bool {
	for i := 0; i < sourceInterfaceType.NumExplicitMethods(); i++ {
		if sourceInterfaceType.ExplicitMethod(i).Name() == "Setup" {
			return true
		}
	}
	return false
